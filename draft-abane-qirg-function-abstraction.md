...
Remote entanglement generation is the fundamental service upon which entanglement-based quantum networking protocols are built. However, exposing low-level entanglement generation primitives directly to applications is insufficient for building interoperable networking systems. To be useful as a network service, entanglement generation should provide three key properties {{pompili-network-stack}}. First, it should be robust, abstracting retries and transient failures so that applications request entanglement rather than individual generation attempts. Second, it should be quantum-platform-independent, allowing applications to request entanglement without depending on the underlying hardware implementation or physical protocol. Third, it should support on-demand operation, enabling entanglement requests to be issued and consumed as part of larger quantum networking protocols. These requirements motivate not only an Entanglement Generation Function (EGF), but more generally the need for reusable network functions that expose well-defined service interfaces while hiding implementation-specific details.
